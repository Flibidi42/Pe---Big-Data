...
Table_clustered <- data.frame(Table_clust_user_date$UserId, k_fit$cluster);
m <- as.matrix(k_fit$centers)

plot(k_fit$centers[1,], col = "red", type="o", ylab="Nb de dépots", xlab="Horaires", 
main="Centers of Clustering (K = 2)");
points(m[2,], col = "green", type = "o");

print("Clustering a 2 cluster:");
for(i in 1:2){
  print(paste("Groupe ", i , " : ", sum(Table_clustered==i)/nrow(Table_clustered)*100, " %"));
}
