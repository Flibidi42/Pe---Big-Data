...
plot(
  x,
  dnorm(x, Mean, Std_dev),
  col = "black",
  type = "l",
  main = "Loi normale pour utilisateurs"
)
...
Pb_User <-
  Table_error_users[Table_error_users$Ratio > qnorm(seuil_refus / 100, Mean, Std_dev), ]
...
for (i in 1:nrow(Pb_User)) {
  a <-
    tot_dep[tot_dep$typeD != 1 &
              as.character(tot_dep$IdUserD) == as.character(Pb_User[i, ]$Users), ]$typeD
  
  tt <- table (a)
  
  Causes_pbs <-
    c(Causes_pbs, causes[as.integer(names(which.max(tt))) + 1])
